...
const WidgetFollow = () => {
  ...
  return (
    <div className='widget border border-gray200 p-10 mb-10'>
      ...
      <ul className='flex flex-wrap justify-center gap-5'>
        {socialMedia.map((item, key) => {
          return (
            <li key={key}>
              <a
                href={`${item.url}`}className={`icon size-10 rounded-full ${item.color} grid place-content-center`}>{item.icon}</a>
            </li>
          );
        })}
      </ul>
